...
namespace RTBEditor {
                             ...
                             int step)
        {
            ...
            for (int z = 0; z < grid.GetHeight(); z += step) {
                for (int x = 0; x < grid.GetWidth(); x += step) {
                    ...
                    center.y += yOffset;
                    const RTBEngine::Math::Vector4& color =
                        grid.IsWalkable(x, z) ? walkableColor : blockedColor;
    ...
    NavGridDebugRenderer::NavGridDebugRenderer()
    {
        ...
        RTBEngine::Navigation::SetNavGridDebugEnabled(true);
    }
    ...
    {
        if (!camera || !lineShader || !RTBEngine::Navigation::IsNavGridDebugEnabled()) {
            return;
        }
        ...
        constexpr float kYOffset = 0.15f;

        if (navGridComponent) {
            AppendBounds(vertices, *navGridComponent, boundsColor, kYOffset);

            if (navGridComponent->IsBaked()) {
                const int step = navGridComponent->GetGrid().GetWidth() > 48 ? 2 : 1;
        ...
        for (RTBEngine::ECS::NavAgentComponent* agent : registeredAgents) {
            ...
            if (!agent->GetWaypoints().empty()) {
                AppendAgentPath(vertices, *agent, pathColor, waypointColor, destinationColor, kYOffset);
            }
        }

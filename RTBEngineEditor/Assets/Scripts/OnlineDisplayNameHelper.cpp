...
namespace GameNet {
    ...
    namespace {
        ...
        void LogPlayerNameFallbackOnce(const std::string& contextKey, const std::string& fallbackName)
        {
            static std::unordered_set<std::string> loggedFallbacks;
            if (!loggedFallbacks.insert(contextKey).second) {
                return;
            }

            RTB_ERROR(
                "[PlayerDisplayName] No se ha encontrado nombre del jugador"
                + (contextKey.empty() ? "." : " (" + contextKey + ").")
                + " Usando fallback: '" + fallbackName + "'.");
        }
    ...
    std::string ResolveLocalDisplayName()
    {
        ...
        LogPlayerNameFallbackOnce("local", "Player");
        return "Player";
    }
    ...
    std::string ResolvePlayerDisplayName(int playerSlot)
    {
        ...
        const std::string fallbackName = "Player " + std::to_string(playerSlot + 1);
        LogPlayerNameFallbackOnce("slot:" + std::to_string(playerSlot), fallbackName);
        return fallbackName;
    }
